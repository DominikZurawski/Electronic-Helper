...
std::string load_psu_symmetric_template(QString& label_out) {
  ...
  if (template_path.isEmpty()) template_path = settings.value("ltspice/template_asc", "").toString();
...
std::string strip_header(const std::string& asc) {
  ...
  while (std::getline(in, line)) {
    ...
    if (line.rfind("SHEET", 0) == 0) continue;
    if (!first) out << "\n";
    out << line;
    first = false;
  }
  ...
  while (std::getline(in, line)) {
    ...
    if (head == "WIRE") {
      ...
      if (iss >> x1 >> y1 >> x2 >> y2) {
        ...
        x2 += dx;
        y2 += dy;
        append_line("WIRE " + std::to_string(x1) + " " + std::to_string(y1) + " " + std::to_string(x2) + " " +
                    std::to_string(y2));
        continue;
      }
    } else if (head == "FLAG") {
      ...
      if (iss >> sym >> x >> y) {
        ...
        x += dx;
        y += dy;
        std::string rebuilt = "SYMBOL " + sym + " " + std::to_string(x) + " " + std::to_string(y) + rest;
        append_line(rebuilt);
        continue;
      }
    } else if (head == "TEXT") {
...
std::string uniquify_instnames(const std::string& asc, const std::string& suffix) {
  ...
  auto apply_suffix = [&](const std::string& name) -> std::string {
    if (name.size() >= suffix.size() && name.rfind(suffix) == name.size() - suffix.size()) return name;
    return name + suffix;
  };
        ...
        if (!directive.empty() && (directive[0] == 'K' || directive[0] == 'k')) {
          ...
          if (toks.size() >= 4) {
            ...
            for (std::size_t i = 1; i + 1 < toks.size(); ++i) {
              ...
              if (it != inst_map.end()) toks[i] = it->second;
            }
...
std::string uniquify_flag_nets(const std::string& asc,
...
std::optional<std::string> prompt_tran_directive(QWidget* parent, bool has_amplifier) {
  ...
  expl->setText(
      "<div style='font-size:14px; font-weight:600; margin:0 0 4px 0;'>Do czego służy <code>.tran</code>?</div>"
      ...
      "</ul>"
      "<div style='margin:0;'>Możesz pominąć <code>.tran</code> w eksporcie i dopisać ją później w LTspice.</div>");
  ...
  auto compute = [&]() -> QString {
    ...
    if (p == "psu") return ".tran 0 2500m 2460m 10u";
    if (p == "audio") return ".tran 0 20m 0 1u";
    if (p == "auto") return has_amplifier ? ".tran 0 20m 0 1u" : ".tran 0 2500m 2460m 10u";
  ...
  auto refresh = [&]() {
    ...
    if (!is_custom && on) custom->setText(compute());
  ...
  if (!include->isChecked()) return std::nullopt;
...
AscAssembly export_project_asc(const std::vector<Block>& blocks,
  ...
  auto add_endpoint = [&](const Block& b, const PortDef& p) {
    ...
    if (ep_to_index.find(key) != ep_to_index.end()) return;
  ...
  for (const auto& c : connections) {
    ...
    if (ita == ep_to_index.end() || itb == ep_to_index.end()) continue;
  ...
  auto base_for_group = [&](const std::vector<EpInfo>& eps) -> std::string {
    ...
    for (const auto& e : eps) {
      if (e.type == PortType::PowerPos) has_vcc = true;
      if (e.type == PortType::PowerNeg) has_vee = true;
      if (e.type == PortType::AnalogIn) has_in = true;
      if (e.type == PortType::AnalogOut) has_out = true;
    }
    if (has_vcc && has_vee) {
      out.warnings.push_back("Sprzeczne typy w jednej sieci (Vcc i Vee) — nadaję nazwę automatyczną.");
      return "NET";
    }
    if (has_vcc) return "Vcc";
    if (has_vee) return "Vee";
    if (has_in && has_out) return "SIG";
    if (has_in) return "IN";
  ...
  // Group blocks into connected components (even power-only ties count) and place each component on its own row.
  std::unordered_map<int, int> id_to_idx;
  id_to_idx.reserve(blocks.size());
  for (int i = 0; i < static_cast<int>(blocks.size()); ++i) id_to_idx.emplace(blocks[i].id, i);
  ...
  for (const auto& c : connections) {
    ...
    if (ia == id_to_idx.end() || ib == id_to_idx.end()) continue;
  ...
  for (int i = 0; i < static_cast<int>(blocks.size()); ++i) comps[comp[i]].push_back(i);
  ...
  for (auto& v : fwd) {
    std::sort(v.begin(), v.end(), [&](int lhs, int rhs) { return blocks[lhs].id < blocks[rhs].id; });
  ...
  auto component_order = [&](const std::vector<int>& indices) -> std::vector<int> {
    ...
    for (int idx : indices) in_comp.emplace(idx, true);
    ...
    for (int idx : indices) {
      if (blocks[idx].kind == "power") starts.push_back(idx);
    }
    std::sort(starts.begin(), starts.end(), [&](int lhs, int rhs) { return blocks[lhs].id < blocks[rhs].id; });
    for (int s : starts) visit_from(s);

    std::vector<int> rest = indices;
    std::sort(rest.begin(), rest.end(), [&](int lhs, int rhs) { return blocks[lhs].id < blocks[rhs].id; });
    for (int s : rest) visit_from(s);
  ...
  auto export_fragment = [&](int dx,
    ...
    for (const auto& kv : xy_to_net) reserved_nets.insert(kv.second);
    const std::string flags_unique = uniquify_flag_nets(patched_flags.asc, inst_suffix, reserved_nets);
      ...
      if (line.rfind("TEXT ", 0) == 0) {
        ...
        if (bang != std::string::npos && bang + 1 < line.size()) {
          ...
          if (!directive.empty() && directive.front() == '.') {
            if (directive.rfind(".tran", 0) == 0) continue;
            if (seen_directives.find(directive) != seen_directives.end()) continue;
  ...
  for (const auto& indices : comps) {
    ...
    for (const int bi : order) {
      ...
      if (b.kind == "power") {
        ...
        out.warnings.insert(out.warnings.end(), patched_values.warnings.begin(), patched_values.warnings.end());
        ...
        for (const auto& p : ports_for(b)) {
          ...
          for (const auto& flag : p.flags) {
            const long long key = (static_cast<long long>(flag.x) << 32) ^ (static_cast<unsigned int>(flag.y));
            xy_to_net.emplace(key, net);
          }
        }

        export_fragment(dx, dy, row_cursor_x, row_max_h, patched_values.asc, xy_to_net, "_B" + std::to_string(b.id));
        continue;
      }
      ...
      if (b.kind == "amplifier") {
        ...
        if (tpl.empty()) {
          ...
          const int ny = row_y + pad_y;
          std::ostringstream note;
          note << "TEXT " << nx << " " << ny << " Left 2 !PPE: brak schematu dla klocka \"" << b.title.toStdString()
               << "\"";
          lines.push_back(note.str());
          out.warnings.push_back("Brak szablonu LTspice dla klocka: " + b.title.toStdString());
          row_cursor_x += (700 + gap_x);
          row_max_h = std::max(row_max_h, 220);
          global_max_x = std::max(global_max_x, nx + 700);
          global_max_y = std::max(global_max_y, ny + 200);
        ...
        if (amp > 0.0 && hz > 0.0) {
          if (b.signal_waveform == "square") {
            ...
            inst_values.emplace("V2",
                                "PULSE(" + fmt(-amp, 6) + " " + fmt(amp, 6) + " 0 1u 1u " + fmt(ton, 9) + " " +
                                    fmt(t, 9) + ")");
          } else if (b.signal_waveform == "triangle") {
            const double t = 1.0 / hz;
            inst_values.emplace("V2",
                                "PWL(0 0 " + fmt(0.25 * t, 9) + " " + fmt(amp, 6) + " " + fmt(0.75 * t, 9) + " " +
                                    fmt(-amp, 6) + " " + fmt(t, 9) + " 0)");
          } else {
