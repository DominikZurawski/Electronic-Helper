...
Widget::Widget(QWidget* parent) : QWidget(parent) {
  ...
  impl_->blocks.push_back(make_power_block(impl_->next_block_id++, "Zasilanie #1", "psu_symmetric"));
  ...
  auto* connections_box = new QGroupBox("Połączenia portów", right);
  auto* connections_layout = new QVBoxLayout(connections_box);
  auto* connections_list = new QListWidget(connections_box);
  ...
  auto* conn_from_block = new QComboBox(connections_box);
  auto* conn_from_port = new QComboBox(connections_box);
  auto* conn_to_block = new QComboBox(connections_box);
  auto* conn_to_port = new QComboBox(connections_box);
  ...
  auto* add_conn = new QPushButton("Dodaj połączenie", connections_box);
  auto* remove_conn = new QPushButton("Usuń zaznaczone połączenie", connections_box);

  connections_layout->addLayout(conn_form);
  connections_layout->addWidget(add_conn);
  connections_layout->addWidget(remove_conn);

  auto* active_title = new QLabel("Aktywny klocek: —", left);
  active_title->setStyleSheet("font-weight: 600; font-size: 14px;");
  impl_->active_title = active_title;

  auto* ports_label = new QLabel(left);
  ...
  power_form->addRow("Wariant", variant);
  power_form->addRow("Uwe (VAC)", vin_input);
  power_form->addRow("Częstotliwość (Hz)", freq_input);
  power_form->addRow("I obciążenia (A)", current_input);
  power_form->addRow("C (uF)", cap_input);
  power_form->addRow("Dodatkowe szyny (+)", extra_rails_input);
  ...
  amp_form->addRow("Wariant", amp_model);
  amp_form->addRow("Zasilanie z", amp_power_source);
  amp_form->addRow("Przebieg wejścia", amp_waveform);
  amp_form->addRow("Amplituda (V)", amp_amp_input);
  amp_form->addRow("Częstotliwość (Hz)", amp_freq_input);
  amp_form->addRow("Wzmocnienie (V/V)", amp_gain_input);
  ...
  auto* export_active = new QPushButton("Eksport aktywnego klocka (.asc)", left);
  auto* export_project = new QPushButton("Eksport całego układu (.asc)", left);

  left_layout->addWidget(active_title);
  left_layout->addWidget(ports_label);
  left_layout->addWidget(props_stack);
  left_layout->addWidget(compute_result);
  left_layout->addWidget(validation);
  left_layout->addWidget(export_active);
  left_layout->addWidget(export_project);
  left_layout->addStretch(1);

  right_layout->addWidget(waveforms_box);
  right_layout->addWidget(new QLabel(
      "Schemat blokowy (kliknij port → port; Ctrl+scroll = zoom; Ctrl+0 reset). Zasilanie pokazuje klocek — bez linii.",
      right));
  right_layout->addWidget(canvas_toolbar);
  right_layout->addWidget(view);
  right_layout->addWidget(connections_box);
  ...
  auto active_block = [this]() -> Block* {
    if (!impl_->active_block_id.has_value()) return nullptr;
  ...
  auto refresh_connection_form = [this]() {
    ...
    if (!conn_from_block || !conn_to_block) return;
    ...
    if (conn_from_block->count() > 0 && conn_from_block->currentIndex() < 0) conn_from_block->setCurrentIndex(0);
    if (conn_to_block->count() > 0 && conn_to_block->currentIndex() < 0) conn_to_block->setCurrentIndex(0);
  };
  ...
  auto refresh_connections_ui = [this]() {
    ...
    for (const auto& c : impl_->connections) {
      ...
      if (!pa.has_value() || !pb.has_value()) continue;
  ...
  auto refresh_canvas = [this]() {
    ...
    auto port_center_for = [this, is_power_type](int block_id, const QString& port_id) -> std::optional<QPointF> {
      ...
      for (const auto& p : ports) {
        if (p.id == port_id) {
          const double r = 6.0;
          const bool is_left =
              (p.type == PortType::AnalogIn) || (p.type == PortType::Ground && b->kind != "power" /* consumer-side */);
    ...
    auto schedule_signal_lines_update = [this]() {
      if (impl_->signal_lines_update_scheduled) return;
      ...
      QTimer::singleShot(0, this, [this]() {
        ...
        auto port_center_for = [this, is_power_type](int block_id, const QString& port_id) -> std::optional<QPointF> {
          ...
          for (const auto& p : ports) {
            if (p.id == port_id) {
              const double r = 6.0;
              const bool is_left =
                  (p.type == PortType::AnalogIn) || (p.type == PortType::Ground && b->kind != "power");
              const double px = (is_left ? 6.0 : (240.0 - 12.0));
              const double py = 12.0 + pin_index * 14.0;
              return b->canvas_pos + QPointF(px + r, py + r);
            }
        ...
        for (auto& sl : impl_->signal_lines) {
          if (!sl.item) continue;
          if (sl.item->scene() != impl_->scene) continue;
    ...
    if (!impl_->manual_layout) {
      ...
      for (int i = 0; i < n; ++i) id_to_idx.emplace(impl_->blocks[i].id, i);
      ...
      auto add_edge = [&](int a_id, int b_id) {
        ...
        if (ia == id_to_idx.end() || ib == id_to_idx.end()) return;
      ...
      for (const auto& c : impl_->connections) {
        ...
        } else if (pa->type == PortType::AnalogIn || pa->type == PortType::AnalogOut || pb->type == PortType::AnalogIn ||
                   pb->type == PortType::AnalogOut) {
          add_edge(ba->id, bb->id);
        }
      }
      ...
      for (const auto& indices : comps) {
        ...
        int col = 0;
        for (int idx : indices) {
          impl_->blocks[idx].canvas_pos = QPointF(margin_x + col * spacing_x, y_cursor + row * spacing_y);
          ++col;
          if (col >= cols) {
            col = 0;
            ++row;
    ...
    for (const auto& b : impl_->blocks) {
      ...
      if (b.kind != "power") {
        ...
        QStringList outs;
        for (const auto& p : ports_for(b)) {
          if (p.type == PortType::Ground) continue;
      ...
      auto* item = new BlockItem(b.id, text, [this, schedule_signal_lines_update](int id, const QPointF& pos) {
        if (impl_->refreshing_canvas) return;
      ...
      scene->addItem(item);
      if (impl_->active_block_id.has_value() && b.id == *impl_->active_block_id) to_select = item;
      ...
      for (const auto& p : ports) {
        ...
        if (p.type == PortType::PowerPos) fill = QColor(255, 210, 210);
        if (p.type == PortType::PowerNeg) fill = QColor(210, 220, 255);
        if (p.type == PortType::Ground) fill = QColor(220, 220, 220);
        if (p.type == PortType::AnalogIn || p.type == PortType::AnalogOut) fill = QColor(210, 245, 210);
    ...
    for (const auto& c : impl_->connections) {
      ...
      const bool a_power = (pa->type == PortType::PowerPos || pa->type == PortType::PowerNeg || pa->type == PortType::Ground);
      const bool b_power = (pb->type == PortType::PowerPos || pb->type == PortType::PowerNeg || pb->type == PortType::Ground);
      if (a_power && b_power) continue; // don't draw power wires on the graph

      const auto a = port_center_for(c.a.block_id, c.a.port_id);
      const auto bpt = port_center_for(c.b.block_id, c.b.port_id);
      if (!a.has_value() || !bpt.has_value()) continue;

      QColor color(0, 140, 70);
      if (!ports_compatible(pa->type, pb->type)) color = QColor(200, 20, 20);

      QPen pen(color, 2.0);
      auto* line = scene->addLine(QLineF(*a, *bpt), pen);
      line->setZValue(-10);
      line->setAcceptedMouseButtons(Qt::NoButton);
      impl_->signal_lines.push_back(Impl::SignalLine{c.a.block_id, c.a.port_id, c.b.block_id, c.b.port_id, line});
    }

    if (to_select) to_select->setSelected(true);
    int max_pins = 0;
    for (const auto& b : impl_->blocks) {
  ...
  auto sync_active_to_form = [this, active_block, refresh_power_source_combo]() {
    ...
    if (!props_stack || !variant || !vin_input || !freq_input || !current_input || !cap_input || !amp_power_source ||
        !extra_rails_input || !amp_waveform || !amp_amp_input || !amp_freq_input || !amp_gain_input) {
      return;
    }
    ...
    if (active->kind == "power") {
      ...
      if (idx >= 0) variant->setCurrentIndex(idx);
      ...
      current_input->setText(active->load_current == 0.0 ? "" : QString::number(active->load_current));
      ...
      for (const auto& c : impl_->connections) {
        ...
        if (!a_is_active && !b_is_active) continue;
        ...
        if (!ap.has_value()) continue;
        if (ap->type != PortType::PowerPos && ap->type != PortType::PowerNeg && ap->type != PortType::Ground) continue;
        selected_psu = ob->id;
        break;
      }
      ...
      if (widx >= 0) amp_waveform->setCurrentIndex(widx);
      amp_amp_input->setText(active->signal_amp_v == 0.0 ? "" : QString::number(active->signal_amp_v));
  ...
  auto sync_form_to_active = [this, active_block]() {
    ...
    auto* amp_freq_input = impl_->amp_freq_input;
    auto* amp_gain_input = impl_->amp_gain_input;
    if (!variant || !vin_input || !freq_input || !current_input || !cap_input || !amp_waveform || !amp_amp_input ||
        !amp_freq_input || !amp_gain_input || !extra_rails_input) {
      return;
    }
  ...
  auto recompute_and_validate = [this, active_block]() {
    ...
    if (!active_title || !compute_result || !validation || !ports_label || !w_in || !w_out) return;

    Block* active = active_block();
    if (!active) {
      active_title->setText("Aktywny klocek: —");
    ...
    active_title->setText(QString("Aktywny klocek: #%1 %2").arg(active->id).arg(active->title));
    ...
    if (active->kind == "power") {
      ...
      compute_result->setText(QString(
          "Podgląd zasilania:\nVpeak: %1 V\nVrect: %2 V\nVdc (z obc.): %3 V\nRipple (Vpp): %4 V")
  ...
  auto connection_exists = [this](const Endpoint& a, const Endpoint& b) -> bool {
    for (const auto& c : impl_->connections) {
      const bool ab = (c.a.block_id == a.block_id && c.a.port_id == a.port_id && c.b.block_id == b.block_id &&
                       c.b.port_id == b.port_id);
      const bool ba = (c.a.block_id == b.block_id && c.a.port_id == b.port_id && c.b.block_id == a.block_id &&
                       c.b.port_id == a.port_id);
      if (ab || ba) return true;
    }
    return false;
  };

  auto try_add_connection = [this, connection_exists](const Endpoint& a, const Endpoint& b) -> bool {
    if (a.block_id == b.block_id && a.port_id == b.port_id) return false;
    if (connection_exists(a, b)) return false;
    impl_->connections.push_back(Connection{a, b});
    return true;
  };

  auto unique_port = [](const std::vector<PortDef>& ports, PortType type) -> std::optional<PortDef> {
  ...
  auto auto_connect_blocks = [this, unique_port, try_add_connection](int from_id, int to_id) {
    ...
    // Never auto-connect PSU <-> PSU (would short supplies) and never auto-connect signal between consumers.
    const bool from_is_psu = (from->kind == "power");
    const bool to_is_psu = (to->kind == "power");
    if (from_is_psu == to_is_psu) return;
  ...
  view->on_add_power = [this,
    ...
    impl_->blocks.push_back(make_power_block(id, QString("Zasilanie #%1").arg(id), "psu_symmetric"));
    ...
    if (impl_->pending_connection.has_value()) {
      ...
      if (start_block && new_block) {
        ...
        if (best_port.isEmpty() && !ports.empty()) best_port = ports.front().id;
  ...
  view->on_delete_selected =
    ...
    impl_->blocks.erase(
        std::remove_if(impl_->blocks.begin(), impl_->blocks.end(), [&](const Block& b) { return b.id == id; }),
        impl_->blocks.end());
    impl_->connections.erase(std::remove_if(impl_->connections.begin(),
  ...
  QObject::connect(btn_add_power, &QPushButton::clicked, this, [view]() {
    if (view->on_add_power) view->on_add_power();
  });
  QObject::connect(btn_add_amp, &QPushButton::clicked, this, [view]() {
    if (view->on_add_amp_model1b) view->on_add_amp_model1b();
  });
  ...
  view->on_port_clicked = [this,
    ...
    if (!impl_->pending_connection.has_value()) {
      ...
      if (impl_->validation) {
        impl_->validation->setText("Połączenia: wybierz drugi port (albo dodaj nowy klocek), aby połączyć.");
      }
      refresh_canvas();
      return;
    }
    ...
    const Endpoint start = *impl_->pending_connection;
    impl_->pending_connection.reset();
    if (start.block_id == ep.block_id && start.port_id == ep.port_id) return;
  ...
  QObject::connect(impl_->amp_power_source, &QComboBox::currentIndexChanged, this, [=, this]() {
    if (impl_->refreshing_canvas) return;
    if (impl_->syncing_active_to_form) return;
    Block* active = active_block();
    if (!active || active->kind == "power") return;
    ...
    auto is_power_endpoint = [this](const Endpoint& e) -> bool {
      ...
      return (p->type == PortType::PowerPos || p->type == PortType::PowerNeg || p->type == PortType::Ground);
    };
    ...
    impl_->connections.erase(std::remove_if(impl_->connections.begin(),
                                            impl_->connections.end(),
                                            [&](const Connection& c) {
                                              if (c.a.block_id != active->id && c.b.block_id != active->id) return false;
    ...
    if (psu_id != 0) {
      ...
      if (psu) {
        ...
        auto pick = [](const std::vector<PortDef>& ports, PortType t) -> QString {
          for (const auto& p : ports) if (p.type == t) return p.id;
          return "";
        };
        ...
        if (!avcc.isEmpty() && !pvcc.isEmpty()) try_add_connection(Endpoint{psu_id, pvcc}, Endpoint{active->id, avcc});
        if (!avee.isEmpty() && !pvee.isEmpty()) try_add_connection(Endpoint{psu_id, pvee}, Endpoint{active->id, avee});
        if (!agnd.isEmpty() && !pgnd.isEmpty()) try_add_connection(Endpoint{psu_id, pgnd}, Endpoint{active->id, agnd});
      }
    }
  ...
  QObject::connect(add_conn, &QPushButton::clicked, this, [=, this]() {
    if (conn_from_block->count() == 0 || conn_to_block->count() == 0) return;
    ...
    if (a_id == b_id && a_port == b_port) return;
    ...
    if (!pa.has_value() || !pb.has_value()) return;
  ...
  QObject::connect(remove_conn, &QPushButton::clicked, this, [=, this]() {
    ...
    if (row < 0 || row >= static_cast<int>(impl_->connections.size())) return;
  ...
  QObject::connect(export_active, &QPushButton::clicked, this, [=, this]() {
    if (!impl_->active_block_id.has_value()) return;
    ...
    for (const auto& c : impl_->connections) {
      if (c.a.block_id == id || c.b.block_id == id) single_conns.push_back(c);
    }
    ...
    if (!exp.warnings.empty()) {
      msg += "\n\nUwagi:\n";
      for (const auto& w : exp.warnings) msg += "- " + QString::fromStdString(w) + "\n";
    }
