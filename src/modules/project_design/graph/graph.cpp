...
BlockItem::BlockItem(int block_id,
                     const QString& text,
...
int BlockItem::block_id() const { return block_id_; }
...
QVariant BlockItem::itemChange(GraphicsItemChange change, const QVariant& value) {
  ...
  if (change == QGraphicsItem::ItemPositionHasChanged) {
    if (on_moved_) on_moved_(block_id_, pos());
  }
  return v;
}
    ...
    if (auto* scene = this->scene()) {
      ...
      if (item) {
        ...
        if (bid.isValid() && pid.isValid()) {
          if (on_port_clicked) on_port_clicked(bid.toInt(), pid.toString());
          event->accept();
          return;
        }
      }
...
void CanvasView::contextMenuEvent(QContextMenuEvent* event) {
  QMenu menu(this);
  auto* add_power = menu.addAction("Dodaj klocek: Zasilanie");
  auto* add_amp = menu.addAction("Dodaj klocek: Wzmacniacz model 1(B)");
  menu.addSeparator();
  auto* del = menu.addAction("Usuń zaznaczony klocek");

  QObject::connect(add_power, &QAction::triggered, this, [&]() {
    if (on_add_power) on_add_power();
  });
  QObject::connect(add_amp, &QAction::triggered, this, [&]() {
    if (on_add_amp_model1b) on_add_amp_model1b();
  });
  QObject::connect(del, &QAction::triggered, this, [&]() {
    if (on_delete_selected) on_delete_selected();
  });
